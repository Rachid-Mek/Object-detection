...
# for all filters
def changeTh(value,img):
    global th
    th = value
    filter(img)
    
# for all filters    
def changeType(x,img):
    global type
    type = x
    filter(img)
    
def changeFilter(value,img,label_values):
    global method
...
def changeStructure(value,img,struct_values):
    global structure
...
# for morphex  
# open and close
def changeMorphType(value,img,morph_values):
    global morph_type
    morph_type = morph_values[value]
    filter(img)

# for erode   
def change_erode_size(x,img):
    global sizeErode
    sizeErode = x 
    filter(img)

# for dilate
def change_dilate_size(x,img):
    global sizeDilate
    sizeDilate = x 
    filter(img)
# for morphex
def change_morphEx_size(x,img):
    global sizeMorphEx
    sizeMorphEx = x 
    filter(img)
...
def sobel_filter(img):
    ...
    gradient_x = filter2D_sobel(sobel_kernel_x,img)
    gradient_y = filter2D_sobel(sobel_kernel_y,img)
    ...
    if method == 'laplacien': 
        ...
        if structure == "rect":
            if morph_type == 'open':
                ...
                morph_rect_closed(img)
        elif structure == "cross":
            if morph_type == 'open':
                morph_cross_open(img)
            elif morph_type == 'close':
                morph_cross_closed(img)
    elif method == 'sobel':
        sobel_filter(img)
    elif method == 'emboss':
        emboss_filter(img) 
...
cv2.createTrackbar("Threshold", "Trackbars", th, 255, lambda x: changeTh(x, img))
cv2.createTrackbar("Type", "Trackbars", type, 4, lambda x: changeType(x, img))
cv2.createTrackbar("sizeErode", "Trackbars", sizeErode, 21, lambda x: change_erode_size(x, img))
cv2.createTrackbar("sizeDilate", "Trackbars", sizeDilate, 21, lambda x: change_dilate_size(x, img))
cv2.createTrackbar("sizeMorph", "Trackbars", sizeMorphEx, 21, lambda x: change_morphEx_size(x, img))
cv2.createTrackbar('Filter', 'Trackbars', 0, len(label_values) - 1, lambda x: changeFilter(x, img, label_values))
cv2.createTrackbar('Struct', 'Trackbars', 0, len(struct_values) - 1, lambda x: changeStructure(x, img, struct_values))
cv2.createTrackbar('Morph', 'Trackbars', 0, len(morph_values) - 1, lambda x: changeMorphType(x, img, morph_values))
